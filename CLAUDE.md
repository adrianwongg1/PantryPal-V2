...
PantryPal Web is a rebuild of a JavaFX desktop app as a hosted, AI-powered recipe app (tell it what's in your kitchen, get a cookable recipe; dietary preferences; a saved pantry; public share links). Status: core loop complete — auth/onboarding, generate, library/detail/edit/share, pantry, preferences, and account settings are all built. This week/meal planning, the Have/Need shopping list, cook mode, voice input, and photo upload are deliberately deferred to a second pass. Stack: Next.js 16 (App Router) + React 19 + TypeScript + Tailwind v4, Supabase (Postgres/auth/storage/RLS), Vercel AI SDK v7 (Groq `openai/gpt-oss-120b` default, Anthropic Claude Haiku 4.5 optional fallback). Package manager is pnpm.
...
### Pantry, Preferences, and Settings (`src/app/(app)/pantry`, `.../preferences`, `.../preferences/settings`)

- `pantry_items.category` is free text with no enum, and nothing before Phase 7 ever set it (onboarding's quick-add, the edit form). `lib/pantry/categories.ts`'s four-value list (Fridge/Freezer/Cupboard/Fresh) is enforced only at the UI layer — a row with an unrecognized or absent category falls into an "Uncategorized" group rather than being hidden or guessed at.
- Adding a pantry item is an **upsert**, not an insert (`onConflict: "user_id,name,status"`) — `pantry_items` has a citext-backed unique `(user_id, name, status)`, and re-adding something already there (a very normal action — "got more milk") would otherwise fail the whole request on a unique violation. Re-adding overwrites quantity/unit/category/expiry with whatever was just typed, not merges with the previous row.
- Settings' Theme row calls `applyTheme`/`storeTheme` (`lib/theme/constants.ts`) directly — the same mechanism `ThemeToggle` uses, applying and storing immediately on click, not gated behind the page's own Save button. The choice is *also* submitted with the rest of the settings form and recorded in `user_preferences.theme`, but nothing reads that column back to decide what to render — doing so would risk the flash-of-wrong-theme the no-flash inline script in `layout.tsx` exists specifically to avoid. Treat that column as a recorded preference for a possible future cross-device sync, not as something in the current render path.
- `user_preferences.default_visibility` is honored in exactly one place: `generate/actions.ts`'s `saveRecipeAction`, the only Server Action that ever creates a recipe from scratch (the edit form's own visibility control only changes an existing row). A `share_slug` is generated up front whenever that default isn't `'private'`, same as the edit action does the first time a recipe leaves private.
- Preferences' "N saved recipes clash" card is a real query, not a guess: `recipes` rows where `diet_tags` does **not** contain every element of the user's current `diets` (PostgREST's `cs`/contains operator, negated via `.not()`). A recipe generated before a diet was added, or generated by a model that didn't tag it correctly, shows up here.
- `delete_own_account()` (Settings' danger zone) deletes the account server-side, but the browser still holds the now-invalid session cookies until they'd naturally expire — the action calls `supabase.auth.signOut()` immediately afterward rather than leaving a stale session sitting client-side.
